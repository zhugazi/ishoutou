(function(a) {
    a.jBox = function(b, c) {
        ...
        j.push('<div class="jbox-container" style="width:auto; height:auto;'+ ('border:'+ c.border2 +'px solid #ccc;') +'">');
        j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:20px; right:20px; width:20px; height:20px;' + (c.showClose ? '': 'display:none;') + '"></a>');
        if (g) {
            j.push('<div class="jbox-title-panel" style="height:60px;">');
            j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon': (c.showIcon == false ? '': ' ' + c.showIcon)) + '" style="float:left; width:80%; line-height:60px; padding-left:' + 0x12 + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;font-size:20px;color:#555;">' + (c.title == '' ? '&nbsp;': c.title) + '</div>');
            j.push('</div>')
        };
        ...
        function(N, O) {
            ...
            a.each(O.buttons, 
            function(T, U) {
                r.push('<button class="jbox-button" value="' + U + '" data="' + U + '" style="' + u + '">' + T + '</button>')
            });
            r.push('</div></div>')
        });
        ...
        function(N, O) {
            ...
            P.children('.jbox-button-panel').children('button').click(function() {
                ...
                if (T === undefined || T) {
                    var nfu = $(this).attr('data');
                    if(nfu.indexOf("jfun_")!= -1) eval(nfu);//如果button是一个函数名，那么就执行此函数
                    else I();
                }
            }).bind('mousedown', 
            function() {
        ...
        var J = function() {
            ...
            if (typeof closeAction != 'undefined' && !c.isTip && !c.isMessager) {
                closeAction();
            }
            ...
            case "POST":
                a.ajax({
                    ...
                    success: function(N, O) {
                        var cob = o.find('#jbox-content');
                        //如果未得到正确数据，则用tip提示
                        if(N.status==0){
                            a.jBox.close();
                            a.jBox.tip(N.message,'fail');
                            return;
                        }
                        ...
                        var newcob = o.find('#jbox-content');
                        //显示加载无素
                        newcob.show().prev().hide();
                        var nwd = newcob.innerWidth();
                        //newcob.parent().css("width",nwd);
                        //最外层元素left位置
                        var Nx = l.width();
                        var Nxe = document.body.clientWidth < Nx ? Nx: document.body.clientWidth;
                        var pleft = (Nxe - nwd)/2;
                        $("#"+c.id).find("#jbox").animate({left: pleft}, 100);
                        L()
                    },
                    error: function() {
                        a.jBox.close();
    ...
    a.jBox.InsertAndResize = function(pa){
            var l = a(window);
            var d = a.jBox.getBox();
            var o = $(d);
            var cob = o.find('#jbox-content');
            ...
            var typet=pa.type||"";
            var urlt=pa.url||"";
            var postdata=pa.postdata||{};
            var buttons=pa.buttons||"";
            ...
            if(buttons!=""){
                ...
                o.find(".jbox-button-panel").html(r.join(''));
                o.find('.jbox-button-panel').children('button').click(function() {
                    var nfu = $(this).attr("data");
                    if(nfu.indexOf("jfun_")!= -1) eval(nfu);//如果button是一个函数名，那么就执行此函数
                    else a.jBox.close();
                }).bind('mousedown', 
            ...
            if(typet!=""){
                var wxs = o.find("#jbox-content").width()+"px";
                o.find("#jbox-content").hide();
                o.find("#jbox-content-loading").css("width",wxs).show();
                a.ajax({
                    type:typet,
                    url: urlt,
                    data: postdata,
                    ...
                    success: function(N, O) {
                        ...
                        var Nxe = document.body.clientWidth < Nx ? Nx: document.body.clientWidth;
                        var pleft = (Nxe - nwd)/2;
                        o.find("#jbox").animate({left: pleft}, 100);
                        //L()
                    }
                })
            }
    ...
    a.jBox.prompt = function(b, c, d, e) {
        ...
        var h = [];
        h.push('html:');
        h.push('<div style="margin:20px;' + g + 'padding-left:' + (e.icon == 'none' ? 0x0: 0x28) + 'px;text-align:left;">');
        h.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:' + (f + e.border+25) + 'px;left:' + (0xa + e.border) + 'px; width:35px; height:36px;"></span>');
        h.push(e.content);
        h.push('</div>');
        e.content = h.join('');
        a.jBox(e.content, e)
    ...
    a.jBox.tip = function(b, c, d) {
        ...
        var e = [];
        e.push('html:');
        e.push('<div style="min-height:18px;height:auto;margin:12px;padding-left:30px;padding-top:0px;text-align:left;">');
        e.push('<span class="jbox-icon jbox-icon-' + d.icon + '" style="position:absolute;top:' + (0x4 + d.border + d.border2-1) + 'px;left:' + (0x4 + d.border + d.border2) + 'px; width:35px; height:36px;"></span>');
        e.push(d.content);
        e.push('</div>');
        d.content = e.join('');
    ...
    a.jBox.messager = function(b, c, d, e) {
        ...
        i.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:' + (g + e.border+25) + 'px;left:' + (0xa + e.border) + 'px; width:35px; height:36px;"></span>');
        i.push(e.content);
        i.push('</div>');
        e.content = i.join('');
        a.jBox(e.content, e)
